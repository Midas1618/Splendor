...
def nguyenlieucannhat(board):
    ...
    soluongcan = 0
    for nguyenlieu in player_02.card_upside_down[0].stocks.keys():
        if player_02.card_upside_down[0].stocks[nguyenlieu] > soluongcan:
            soluongcan = player_02.card_upside_down[0].stocks[nguyenlieu]
...
def nguyenlieucannhi(board):
    ...
    for nguyenlieu in nguyenlieucon(board):
        if player_02.card_upside_down[0].stocks[nguyenlieu] > 0:
            return nguyenlieu
            luongnguyenlieu = -1
            break
        else:
    ...
    if len(player_02.card_upside_down) == 0:
        ...
        for a in board.dict_Card_Stocks_Show["III"]:
            if sum(a.stocks.values()) == 10:
                thesenhat = a
        if thesenhat != None:
            return player_02.getUpsideDown(thesenhat, board, {})
        else:
            IIlonnhat = -1
        ...
        # mua thẻ
        if player_02.checkGetCard(player_02.card_upside_down[0]) == True:
            return player_02.getCard(player_02.card_upside_down[0], board)
        # nhặt nguyên liệu
        # nhặt 2 nguyên liệu
        else:
            if player_02.checkOneStock(board, nguyenlieucannhat(board)) == True:
                return player_02.getOneStock(nguyenlieucannhat(board), board, {})
            else:
                ...
                if sum(player_02.stocks.values()) > 8:
                    ...
                    for the in board.dict_Card_Stocks_Show["III"]:
                        if sum(the.stocks.values()) == 10:
                            theseup == the
                    if theseup != None:
                        return player_02.getUpsideDown(theseup, board, bo1)
                    else:
                        IIlon = 0
            ...
            # nhặt 3 nguyên liệu
                    ...
                    if player_02.checkThreeStocks(board, nguyenlieucannhat(board), nguyenlieucannhi(board), nguyenlieucanba(board)) == True:
                        bo = {}
                        if sum(player_02.stocks.values()) == 8:
                            bo = {nguyenlieucanba(board): 1}
                        return player_02.getThreeStocks(nguyenlieucannhat(board), nguyenlieucannhi(board), nguyenlieucanba(board), board, bo)
