...
import random
import math
player_04 = player.Player("Violet", 0)
...
def list_token_for_Card_down(board):
    ...
    if len(player_04.card_upside_down)>0:
        ...
        for token in list(box.stocks.keys()):
            if(box.stocks[token] - player_04.stocks[token] - player_04.stocks_const[token]) > a:
                if board.stocks[token]>0:
                    a = box.stocks[token]- player_04.stocks[token] - player_04.stocks_const[token]
...
def list_Card_down():
    A = []
    if len(player_04.card_upside_down)>0:
        for object in  player_04.card_upside_down:
            Card_down =  player_04.card_upside_down.copy()
...
def list_token_return(board):
    token_return = []
    for token in player_04.stocks.keys():
        if player_04.stocks[token]>0 and token!= "auto_color":
            if token not in  list_token_for_Card_down(board):
                token_return.append(token)
    item =  list_token_for_Card_down(board).copy()
    item.reverse()
    for token in item:
        if player_04.stocks[token] > 0 and token not in token_return:
            token_return.append(token)
    print(5)
    return token_return
...
def take_card_to_take_noble(board):
    ...
    for card in board.dict_Card_Stocks_UpsiteDown['Noble']:
        ...
        for type_card in card.stocks.keys():
            dict_card_to_take[type_card] = max(card.stocks[type_card] - player_04.stocks_const[type_card], 0)
...
def action(board, arr_player):
    if len(list_card_can_take(board)) > 0:
        print(9)
        return player_04.getCard(list_card_can_take(board)[0],board)
    if len(list_the_ngon(board))>0:
        if player_04.checkUpsiteDown():
             print(10)
             return player_04.getUpsideDown(list_the_ngon(board)[0], board,tra_token)
    if len(player_04.card_upside_down)>0:
        if sum(player_04.stocks.values()) <=8:
            if len(list_token_for_Card_down(board)) > 0:
                print(11)
                if player_04.checkOneStock(board,list_token_for_Card_down(board)[0]):
                     print("12")
                     return player_04.getOneStock(list_token_for_Card_down(board)[0], board, tra_token)
            if len(list_token_for_Card_down(board))>2:
                if player_04.checkThreeStocks(board, list_token_for_Card_down(board)[0], list_token_for_Card_down(board)[1], list_token_for_Card_down(board)[2]):
                    print(13)
                    return player_04.getThreeStocks(list_token_for_Card_down(board)[0], list_token_for_Card_down(board)[1], list_token_for_Card_down(board)[2], board, tra_token)

            if support_card(board) != None:
                    if len(token_support_card(board))> 2:
                           if player_04.checkThreeStocks(board, token_support_card(board)[0], token_support_card(board)[1], token_support_card(board)[2]):
                              print("co len")
                              return player_04.getThreeStocks(token_support_card(board)[0], token_support_card(board)[1], token_support_card(board)[2], board, tra_token)
                    for token in token_support_card(board):
                        if player_04.checkOneStock(board, token):
                            print(14)
                            return player_04.getOneStock(token, board, tra_token)
            else:
