...
def codes(board,player_01):
    ...
    for Noble in board.dict_Card_Stocks_Show["Noble"]:
        code = str(Noble.id)[6:]
        for nl in player_01.stocks.keys():
            code += str(player_01.stocks[nl])
        for nl in player_01.stocks_const.keys():
            code += str(player_01.stocks_const[nl])
...
def act(target,board,player_01,mind_c,pools_c):
    ...
    if sum(missing.values()) > 10:
        return None,player_01.stocks
    else:
        if player_01.checkGetCard(target) == True:
            return "up",target
        can_va_co = []
        ...
        if len(can_va_co) == 0:
            if board.stocks['auto_color'] == 0:
                ...
                if player_01.checkUpsiteDown() == True:
                    if target not in player_01.card_upside_down:
                        return "down",target
                    else:
                        state = False
                        while state == False:
            ...
            for nl in target.stocks.keys():
                if nl not in can_va_co and board.stocks[nl] > 0:
                    dinh_lay.append(nl)
            co_the_lay = min(3,10 - sum(player_01.stocks.values()),len(dinh_lay))
            cbi_lay = []
            for so_luong in range(co_the_lay):
                cbi_lay.append(dinh_lay[so_luong])
                nl_lay[dinh_lay[so_luong]] =1
            if len(cbi_lay) == 3:
                # ac =  player_01.getThreeStocks(nl_lay,board,{})
                return "3", cbi_lay
            if len(cbi_lay) == 2:
                # ac =  player_01.getOneTwoStock(cbi_lay[0],cbi_lay[1],board,{})
                return "2", cbi_lay
            if len(cbi_lay) == 1:
                # ac =  player_01.getOneTwoStock(cbi_lay[0],'Null',board,{})
                return "1",cbi_lay
    return None,None
...
def action(board, arr_player):
    ...
    if ac == "down":
        return player_01.getUpsideDown(b,board,{})
    if ac == "3":
        return player_01.getThreeStocks(b[0],b[1],b[2],board,{})
    if ac == "2":
        return player_01.getOneTwoStock(b[0],b[1],board,{})
    if ac == "1":
