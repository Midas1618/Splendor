...
import operator
import json
player_04 = player.Player("player_04", 0)
import pathlib
import numpy as np
...
def act(target,board,player_04,data,pools_c):
    missing = {}
...
def action(board, arr_player):
    ...
    while ac == None and len(loai) < 12:
        ...
        ac,b = act(target,board,player_04,data,pools_c)
        # time += 1
    # print(target.id)
    learning = pd.read_csv("4l.csv")
    ...
    learning["turn"] = turn
    learning.to_csv("4l.csv",index = False)
    if ac == "up":
