...
import json
import pathlib
player_04 = player.Player("4", 0)
 
#code creation
def codes(board,player_04):
    codes = []
    for Noble in board.dict_Card_Stocks_Show["Noble"]:
        code = str(Noble.id)[6:]
        for nl in player_04.stocks.keys():
            code += str(player_04.stocks[nl])
        for nl in player_04.stocks_const.keys():
            code += str(player_04.stocks_const[nl])
        for nl in board.stocks.keys():
            code += str(board.stocks[nl])
        a = str(player_04.score)
        if len(a) < 2:
            a = "a" + a 
        code += a 
        codes.append(code)
...
#create mind
def mind(board,player_04):
    ds_codes = codes(board,player_04)
    mind = start()
...
def act(target,board,player_04,mind_c,pools_c):
    missing = {}
    for nl in target.stocks.keys():
        thieu = target.stocks[nl] - player_04.stocks[nl] - player_04.stocks_const[nl]
        if thieu > 0:
            missing[nl] = thieu
    if sum(missing.values()) > 10:
        return None,player_04.stocks
    else:
        if player_04.checkGetCard(target) == True:
            return "up",target
        can_va_co = []
        ...
        if len(can_va_co) == 0:
            if board.stocks['auto_color'] == 0:
                ...
                if player_04.checkUpsiteDown() == True:
                    if target not in player_04.card_upside_down:
                        return "down",target
                    else:
                        state = False
                        while state == False:
                            target_2 = decision(board,mind_c,pools_c)
                            state = target_2 not in player_04.card_upside_down
            ...
            for nl in target.stocks.keys():
                if nl not in can_va_co and board.stocks[nl] > 0:
                    dinh_lay.append(nl)
            co_the_lay = min(3,10 - sum(player_04.stocks.values()),len(dinh_lay))
            cbi_lay = []
            for so_luong in range(co_the_lay):
                cbi_lay.append(dinh_lay[so_luong])
                nl_lay[dinh_lay[so_luong]] =1
            if len(cbi_lay) == 3:
                # ac =  player_04.getThreeStocks(nl_lay,board,{})
                return "3", cbi_lay
            if len(cbi_lay) == 2:
                # ac =  player_04.getOneTwoStock(cbi_lay[0],cbi_lay[1],board,{})
                return "2", cbi_lay
            if len(cbi_lay) == 1:
                # ac =  player_04.getOneTwoStock(cbi_lay[0],'Null',board,{})
                return "1",cbi_lay
    return None,None
 
def action(board, arr_player):
    # print(player_04.score)
    ds_codes = codes(board,player_04)
    # print(codes(board,player_04))
    # print(basic())
    # print(pools(board,player_04))
    # print(mind(board,player_04))
    # print(decision(board,player_04))
    mind_c = mind(board,player_04)
    pools_c = pools(board,player_04)
    ...
    while ac == None and time < 16:
        target = decision(board,mind_c,pools_c)
        ac,b = act(target,board,player_04,mind_c,pools_c)
        time += 1
        # print("player 1 nghĩ lần ",time)
    try:
        f = open("p4learning.json")
        dahoc = json.load(f)
    except:
        dahoc = []
    saving = {}
    ...
    if ac == "up":
        return player_04.getCard(b,board)
    if ac == "down":
        return player_04.getUpsideDown(b,board,{})
    if ac == "3":
        return player_04.getThreeStocks(b[0],b[1],b[2],board,{})
    if ac == "2":
        return player_04.getOneTwoStock(b[0],b[1],board,{})
    if ac == "1":
        return player_04.getOneTwoStock(b[0],"Null",board,{})
    return board
