...
def nguyenlieucannhat(board):
    ...
    soluongcan = 0
    for nguyenlieu in player_03.card_upside_down[0].stocks.keys():
        if player_03.card_upside_down[0].stocks[nguyenlieu] > soluongcan:
            soluongcan = player_03.card_upside_down[0].stocks[nguyenlieu]
...
def nguyenlieucannhi(board):
    ...
    for nguyenlieu in nguyenlieucon(board):
        if player_03.card_upside_down[0].stocks[nguyenlieu] > 0:
            return nguyenlieu
            luongnguyenlieu = -1
            break
        else:
...
def moiturn(board):
    if len(player_03.card_upside_down) == 0:
        ...
        # mua thẻ
        if player_03.checkGetCard(player_03.card_upside_down[0]) == True:
            return player_03.getCard(player_03.card_upside_down[0], board)
        # nhặt nguyên liệu
        # nhặt 2 nguyên liệu
        else:
            if player_03.checkOneStock(board, nguyenlieucannhat(board)) == True:
                return player_03.getOneStock(nguyenlieucannhat(board), board, {})
            else:
                ...
                if sum(player_03.stocks.values()) > 8:
                    ...
                    for the in board.dict_Card_Stocks_Show["III"]:
                        if sum(the.stocks.values()) == 10:
                            theseup == the
                    if theseup != None:
                        return player_03.getUpsideDown(theseup, board, bo1)
                    else:
                        IIlon = 0
            ...
            # nhặt 3 nguyên liệu
                    ...
                    if player_03.checkThreeStocks(board, nguyenlieucannhat(board), nguyenlieucannhi(board), nguyenlieucanba(board)) == True:
                        bo = {}
                        if sum(player_03.stocks.values()) == 8:
                            bo = {nguyenlieucanba(board): 1}
                        return player_03.getThreeStocks(nguyenlieucannhat(board), nguyenlieucannhi(board), nguyenlieucanba(board), board, bo)
